...
//HH: Botones de navegacion Compania     
    ...
    public function buscar_compania_primero() {
        ...
        //$query = $this->db->queries("SELECT * FROM compania order by idcompania desc LIMIT 0,1");
        if ($query->num_rows() === 1) {
            return $query->result();
        } else {
            return false;
    ...
    public function buscar_compania_anterior($idcompania_actual) {
        ...
        //echo "SELECT * FROM compania WHERE idcompania < '".$idcompania_actual."' LIMIT 0,1";
        if ($query->num_rows() === 1) {
            return $query->result();
        } else {
            return false;
    ...
    public function buscar_contacto_siguiente($idcompania_actual,$idcontacto_actual) {
        ...
        //echo "SELECT * FROM contacto where idcompania = '".$idcompania_actual."' and idcontacto < '".$idcontacto_actual."' order by idcontacto desc LIMIT 0,1";
        if ($query->num_rows() === 1) {
            return $query->result();
        } else {
            return false;
    ...
    public function buscar_contacto_anterior($idcompania_actual,$idcontacto_actual) {
        ...
        //echo "select * from contacto where idcontacto IN (SELECT idcontacto FROM contacto where idcompania = '".$idcompania_actual."') and  idcontacto < '".$idcontacto_actual."' order by idcontacto asc LIMIT 0,1";
        if ($query->num_rows() === 1) {
            return $query->result();
        } else {
            return false;
